...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  setwd("C:/Users/Mohit Singh/Desktop/R language")
...
# Store the movie name in variable named top_movie
	  ...
	  sort_1 <- sort(bollywood$Tcollection,decreasing = TRUE)
	  top_movie <- bollywood$Movie[which(bollywood$Tcollection==sort_1[1])] 
    top_movie 
...
# Store the movie name in variable named top_2_movie
    ...
    sort_1 <- sort(bollywood$Tcollection,decreasing = TRUE)
	  top_2_movie <- bollywood$Movie[which(bollywood$Tcollection==sort_1[2])]
    top_2_movie 
...
# Write command/s to find out how many movies are in Flop, Average, Hit and Superhit categories in the entire Bollywood data set.
	...
	hit
	super_hit<-length(which(bollywood$Verdict=="Super Hit"))
	super_hit
...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
	max_value<-apply(bollywood,2,max,na.rm = TRUE)
	max_value
	
#Q7 
# Write a command to find the names of the movies which have the maximum Ocollection, Wcollection, Fwcollecion & Tcollection

	max_ocollect <- bollywood$Movie[which(bollywood$Ocollection==max_value[4])]
	max_ocollect
	max_wcollect <- bollywood$Movie[which(bollywood$Wcollection==max_value[5])]
	max_wcollect
	max_fwcollect <- bollywood$Movie[which(bollywood$Fwcollection==max_value[6])]
	max_fwcollect
	max_tcollect <- bollywood$Movie[which(bollywood$Tcollection==max_value[7])]
	max_tcollect
	
# Store the names of 4 movies in same sequence in movie_result vector

  movie_result <-(c(max_ocollect,max_wcollect,max_fwcollect,max_tcollect))
  movie_result
